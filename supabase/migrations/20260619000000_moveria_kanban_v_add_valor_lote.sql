-- =============================================================================
-- Migration: moveria_kanban_v_add_valor_lote
-- Banco: Moveria (fzgasvcfxufhrbrdakow) APENAS
-- Adiciona coluna valor_lote (numeric) como col 24 ao final de cada branch.
--   Branch 1 (backlog):      NULL::numeric AS valor_lote
--   Branch 2 (aguardando):   NULL::numeric AS valor_lote
--   Branch 3 (lotes):        SUM(valor_item) dos itens do lote (deletado_em IS NULL)
-- Guard: moveria_designacoes ausente → pula (Core/Mowig).
-- Sem alteração em filtros RLS, ramos ou colunas existentes.
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'kanban_v_add_valor_lote: moveria_designacoes ausente — pulada.'; RETURN;
  END IF;
END $guard$;
...
SELECT
  ...
  (SELECT SUM(i.valor_item)
   ...
   WHERE i.lote_id = l.id AND i.deletado_em IS NULL
  )                         AS valor_lote
...
VALUES ('20260619000000_moveria_kanban_v_add_valor_lote.sql')
