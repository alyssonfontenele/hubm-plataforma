-- =============================================================================
-- Migration: moveria_kanban_v_add_cliente_id
-- Banco: Moveria (fzgasvcfxufhrbrdakow) APENAS
-- Estende moveria_kanban_v com cliente_id uuid como col 23 (ao final de cada branch).
--   Branch 1/2: cl.id AS cliente_id (já tem JOIN moveria_clientes cl)
--   Branch 3:   l.cliente_id (moveria_lotes_v já expõe)
-- Guard: moveria_designacoes ausente → pula (Core/Mowig).
-- Tipo de operação: CREATE OR REPLACE VIEW apenas. Sem DDL de tabela.
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'kanban_v_add_cliente_id: moveria_designacoes ausente — pulada.'; RETURN;
  END IF;
END $guard$;
...
VALUES ('20260604070000_moveria_kanban_v_add_cliente_id.sql')
